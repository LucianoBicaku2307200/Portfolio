import { ResourceType } from "../types";

export const resourcesData: ResourceType[] = [
	{
		label: "CambioBike",
		description:
			"Discover a vast array of bicycles and accessories for cycling enthusiasts at CambioBike's online store.",
		href: "https://cambiobike.it/",
		tags: ["#bigCommerce", "#eurostep SRL"],
		platforms: ["bigcommerce"],
		technologies: ["Stencil JS", "Scss", "React Js", "HandleBars"],
		repoUrl: "",
		className: "md:col-start-1 md:col-end-3 md:row-start-1 md:row-end-3",
	},
	{
		label: "Miss Bikini",
		description:
			"Miss Bikini brings you an exquisite collection of stylish swimwear and beachwear for the fashion-forward.",
		href: "https://missbikini.eu/",
		tags: ["#bigCommerce", "#eurostep SRL"],
		platforms: ["bigcommerce"],
		technologies: ["Stencil JS", "Scss", "React Js", "HandleBars"],
		repoUrl: "",
		className: "md:col-start-3 md:col-end-4 md:row-start-1 md:row-end-3"
	},
	{
		label: "Alemaco",
		description: "Experience the epitome of Italian craftsmanship and elegance through Alemaco's meticulously crafted furniture designs.",
		href: "https://alemaco.it/",
		tags: ["#bigCommerce", "#eurostep SRL"],
		platforms: ["bigcommerce"],
		technologies: ["Stencil JS", "Scss", "React Js", "HandleBars"],
		repoUrl: "",
		className: "md:col-start-1 md:col-end-2 md:row-start-3 md:row-end-5"
	},
	{
		label: "JohnyLambs",
		description: "Step into the world of Johnny Lambs and indulge in uniquely designed urban footwear with contemporary aesthetics.",
		href: "https://en.johnnylambs.com/",
		tags: ["#bigCommerce", "#eurostep SRL"],
		platforms: ["bigcommerce"],
		technologies: ["Stencil JS", "Scss", "React Js", "HandleBars"],
		repoUrl: "",
		className: "md:col-start-2 md:col-end-4"
	},
	{
		label: "Times",
		description: "Feel the Fashion's trendy clothing collection sets the stage for style-conscious individuals to express their unique fashion sense.",
		href: "https://feelthefashion.it/",
		tags: ["#bigCommerce", "#eurostep SRL"],
		platforms: ["bigcommerce"],
		technologies: ["Stencil JS", "Scss", "React Js", "HandleBars"],
		repoUrl: "",
		className: ""
	}
];
